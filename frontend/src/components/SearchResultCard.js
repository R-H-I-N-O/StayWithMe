        ...
        <div>
            <div className="grid grid-cols-1 xl:grid-cols-[2fr_3fr] border border-slate-300 rounded-lg p-8 gap-8">
                ...
                <div className="grid grid-rows-[1fr_2fr_1fr]">
                    ...
                    <Link to={`/details/${hotel._id}`} className='text-2xl font-bold'>{hotel.name}</Link>
                </div>
                <div>
                    <div className='line-clamp-4'>{hotel.description}</div>
                </div>
                <div className='grid grid-cols-2 items-end whitespace-nowrap pt-12'>
                    <div className='flex gap-1 items-center'>
                        ...
                        <span className='text-sm'> {hotel.facilities?.length > 0 && `+${hotel.facilities.length - 2} more`}</span>
                    </div>
